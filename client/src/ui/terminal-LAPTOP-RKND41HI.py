...
class Terminal:
    ...
    def start(self):
        """
        Starts the terminal interface and launches the user input thread.
        """
    ...
    def input_loop(self):
        """
        Continuously reads user input and dispatches commands or chat actions.
        """
        while self.running:
            text = input("> ").strip()
    ...
    def process_unsent_batch(self, groups):
        """
        Processes and queues unread messages retrieved from storage.
        """
    ...
    def process_msg(self, message, channel):
        """
        Processes an incoming message and routes it to the active chat
        or unread message queue.
        """
    ...
    def process_unread_in_current_chat(self):
        """
        Displays queued unread messages for the current chat session.
        """
    ...
    def queue_msg(self, message):
        """
        Stores a message in the unread message queue.
        """
    ...
    def notify_msg(self, message):
        """
        Displays a notification for a newly received message.
        """
        data = message.get("data")
        from_user = data.get("from")
        chat_type = data.get("chat_type")
    ...
    def print_msg(self, message, is_unread=False):
        ...
        Args:
            ...
            is_unread (bool): Indicates if the message was previously unread.
        """
        data = message.get("data")
        from_user = data.get("from")
        if is_unread:
    ...
    def load_private_logs(self, chat_id):
        """
        Loads and displays message history for a private chat.
        """
    ...
    def load_group_logs(self, chat_id):
        """
        Loads and displays message history for a group chat.
        """
    ...
    def accept_transfer(self, transfer_id):
        ...
        Args:
            transfer_id (int): Identifier of the file transfer.
        """
    ...
    def resume(self):
        """
        Resumes command processing after waiting for a response.
        """
        self.wait_event.set()
    ...
    def show_logged_in_menu(self):
        """
        Displays the main chat menu for authenticated users.
        """
        self.clear()
        print("=== CHAT MENU ===")
        print("1. Enter Private Chat")
    ...
    def login(self):
        """
        Prompts the user for credentials and sends an authentication request.
        """
    ...
    def register(self):
        """
        Prompts the user for account details and sends a registration request.
        """
    ...
    def quit(self):
        """
        Terminates the terminal interface and closes the client connection.
        """
        self.running = False
        self.logout()
        self.on_user_input({
